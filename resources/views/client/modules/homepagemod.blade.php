@extends('client/layouts/homepagelay')
@section('content')
<div class="container-fluid" style="border: 0.1px solid gray;background-color: white; margin-top:10px;margin-bottom:10px;box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">
    <div style="margin-bottom:10px">
        <h1 style="font-size: 25px;display:inline">Featured Categories</h1> <a style="display: inline;" href="">View All</a>
    </div>
    <div id="carouselExampleControls" style="margin-top: 0px;" class="carousel slide" data-ride="carousel">
        <div class="carousel-inner">
            <div class="carousel-item active">
                <div class="row">
                    <div class="col-md-4">

                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>



                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/2.jpg" alt="First slide">
                            <div class="card-body">

                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/3.jpg" alt="First slide">
                            <div class="card-body">

                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>


                </div>

            </div>
            <div class="carousel-item">
                <div class="row">
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>


                </div>
            </div>
            <div class="carousel-item">
                <div class="row">
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>


                </div>
            </div>
        </div>
        <a class="carousel-control-prev" href="#carouselExampleControls" role="button" data-slide="prev">
            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
            <span class="sr-only">Previous</span>
        </a>
        <a class="carousel-control-next" href="#carouselExampleControls" role="button" data-slide="next">
            <span class="carousel-control-next-icon" aria-hidden="true"></span>
            <span class="sr-only">Next</span>
        </a>
    </div>

</div>

<style>
    .mainclass {
  position: relative;
  width: 50%;
}

.image {
  opacity: 1;
  display: block;
  width: 100%;
  height: auto;
  transition: .5s ease;
  backface-visibility: hidden;
}

.middle {
  transition: .5s ease;
  opacity: 0;
  position: absolute;
  top: 50%;
  left: 50%;
  transform: translate(-50%, -50%);
  -ms-transform: translate(-50%, -50%);
  text-align: center;
}

.mainclass:hover .image {
  opacity: 0.3;
}

.mainclass:hover .middle {
  opacity: 1;
}


</style>

<div class="container-fluid" style="margin-top:10px;margin-bottom:10px;background-color: white; margin-top:10px;margin-bottom:10px;box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">
    <h1 style="font-size: 25px;display:inline;">Featured Products</h1>
    <hr style="border: 1px solid red;">
    <div class="card mainclass" style="width:20rem;display:inline-block;margin:3px">
        <img class="d-block w-100 image" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
        <div style="text-align: center;" class="card-body ">
            <button class="text middle btn btn-danger" style="margin-top: -50px;">Add To Cart</button>
            <p class="card-text">Some quick example text Some quick example text Some quick example text Some quick example text </p>
            <p style="line-height: 0;">VAVA</p>
           <p style="line-height: 1;">141 reviews</p>
           <p style="color: red; line-height:0">R.s 7,999.00</p>
        </div>
    </div>
    <div class="card mainclass" style="width:20rem;display:inline-block;margin:3px">
        <img class="d-block w-100 image" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
        <div style="text-align: center;" class="card-body ">
            <button class="text middle btn btn-danger" style="margin-top: -50px;">Add To Cart</button>
            <p class="card-text">Some quick example text Some quick example text Some quick example text Some quick example text </p>
            <p style="line-height: 0;">VAVA</p>
           <p style="line-height: 1;">141 reviews</p>
           <p style="color: red; line-height:0">R.s 7,999.00</p>
        </div>
    </div>
    <div class="card mainclass" style="width:20rem;display:inline-block;margin:3px">
        <img class="d-block w-100 image" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
        <div style="text-align: center;" class="card-body ">
            <button class="text middle btn btn-danger" style="margin-top: -50px;">Add To Cart</button>
            <p class="card-text">Some quick example text Some quick example text Some quick example text Some quick example text </p>
            <p style="line-height: 0;">VAVA</p>
           <p style="line-height: 1;">141 reviews</p>
           <p style="color: red; line-height:0">R.s 7,999.00</p>
        </div>
    </div>
    <div class="card mainclass" style="width:20rem;display:inline-block;margin:3px">
        <img class="d-block w-100 image" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
        <div style="text-align: center;" class="card-body ">
            <button class="text middle btn btn-danger" style="margin-top: -50px;">Add To Cart</button>
            <p class="card-text">Some quick example text Some quick example text Some quick example text Some quick example text </p>
            <p style="line-height: 0;">VAVA</p>
           <p style="line-height: 1;">141 reviews</p>
           <p style="color: red; line-height:0">R.s 7,999.00</p> 
        </div>
    </div>

</div>

<div class="container-fluid" style="margin-top:10px;margin-bottom:10px;background-color: white; margin-top:10px;margin-bottom:10px;box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">
    <h1 style="font-size: 25px;display:inline;">Top Products</h1>
    <hr style="border: 1px solid red;">
    <div class="card mainclass" style="width:20rem;display:inline-block;margin:3px">
        <img class="d-block w-100 image" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
        <div style="text-align: center;" class="card-body ">
            <button class="text middle btn btn-danger" style="margin-top: -50px;">Add To Cart</button>
            <p class="card-text">Some quick example text Some quick example text Some quick example text Some quick example text </p>
            <p style="line-height: 0;">VAVA</p>
           <p style="line-height: 1;">141 reviews</p>
           <p style="color: red; line-height:0">R.s 7,999.00</p>
        </div>
    </div>
    <div class="card mainclass" style="width:20rem;display:inline-block;margin:3px">
        <img class="d-block w-100 image" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
        <div style="text-align: center;" class="card-body ">
            <button class="text middle btn btn-danger" style="margin-top: -50px;">Add To Cart</button>
            <p class="card-text">Some quick example text Some quick example text Some quick example text Some quick example text </p>
            <p style="line-height: 0;">VAVA</p>
           <p style="line-height: 1;">141 reviews</p>
           <p style="color: red; line-height:0">R.s 7,999.00</p>
        </div>
    </div>
    <div class="card mainclass" style="width:20rem;display:inline-block;margin:3px">
        <img class="d-block w-100 image" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
        <div style="text-align: center;" class="card-body ">
            <button class="text middle btn btn-danger" style="margin-top: -50px;">Add To Cart</button>
            <p class="card-text">Some quick example text Some quick example text Some quick example text Some quick example text </p>
            <p style="line-height: 0;">VAVA</p>
           <p style="line-height: 1;">141 reviews</p>
           <p style="color: red; line-height:0">R.s 7,999.00</p>
        </div>
    </div>
    <div class="card mainclass" style="width:20rem;display:inline-block;margin:3px">
        <img class="d-block w-100 image" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
        <div style="text-align: center;" class="card-body ">
            <button class="text middle btn btn-danger" style="margin-top: -50px;">Add To Cart</button>
            <p class="card-text">Some quick example text Some quick example text Some quick example text Some quick example text </p>
            <p style="line-height: 0;">VAVA</p>
           <p style="line-height: 1;">141 reviews</p>
           <p style="color: red; line-height:0">R.s 7,999.00</p> 
        </div>
    </div>

</div>


<div class="container-fluid" style="margin-top:10px;margin-bottom:10px;background-color: white; margin-top:10px;margin-bottom:10px;box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">
    <h1 style="font-size: 25px;display:inline;">New Arrivals </h1>
    <hr style="visibility:hidden">
    <div class="card mainclass" style="width:20rem;display:inline-block;margin:3px">
        <img class="d-block w-100 image" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
        <div style="text-align: center;" class="card-body ">
            <button class="text middle btn btn-danger" style="margin-top: -50px;">Add To Cart</button>
            <p class="card-text">Some quick example text Some quick example text Some quick example text Some quick example text </p>
            <p style="line-height: 0;">VAVA</p>
           <p style="line-height: 1;">141 reviews</p>
           <p style="color: red; line-height:0">R.s 7,999.00</p>
           <button class="btn btn-danger">Add To Cart</button>
        </div>
    </div>
    <div class="card mainclass" style="width:20rem;display:inline-block;margin:3px">
        <img class="d-block w-100 image" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
        <div style="text-align: center;" class="card-body ">
            <button class="text middle btn btn-danger" style="margin-top: -50px;">Add To Cart</button>
            <p class="card-text">Some quick example text Some quick example text Some quick example text Some quick example text </p>
            <p style="line-height: 0;">VAVA</p>
           <p style="line-height: 1;">141 reviews</p>
           <p style="color: red; line-height:0">R.s 7,999.00</p>
           <button class="btn btn-danger">Add To Cart</button>
        </div>
    </div>
    <div class="card mainclass" style="width:20rem;display:inline-block;margin:3px">
        <img class="d-block w-100 image" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
        <div style="text-align: center;" class="card-body ">
            <button class="text middle btn btn-danger" style="margin-top: -50px;">Add To Cart</button>
            <p class="card-text">Some quick example text Some quick example text Some quick example text Some quick example text </p>
            <p style="line-height: 0;">VAVA</p>
           <p style="line-height: 1;">141 reviews</p>
           <p style="color: red; line-height:0">R.s 7,999.00</p>
           <button class="btn btn-danger">Add To Cart</button>
        </div>
    </div>
    <div class="card mainclass" style="width:20rem;display:inline-block;margin:3px">
        <img class="d-block w-100 image" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
        <div style="text-align: center;" class="card-body ">
            <button class="text middle btn btn-danger" style="margin-top: -50px;">Add To Cart</button>
            <p class="card-text">Some quick example text Some quick example text Some quick example text Some quick example text </p>
            <p style="line-height: 0;">VAVA</p>
           <p style="line-height: 1;">141 reviews</p>
           <p style="color: red; line-height:0">R.s 7,999.00</p> 
           <button class="btn btn-danger">Add To Cart</button>
        </div>
    </div>

</div>

<div class="container-fluid" style="border: 0.1px solid gray;background-color: white; margin-top:10px;margin-bottom:10px;box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">
    <div style="margin-bottom:10px; padding-top:10px">
        <h1 style="font-size: 25px;display:inline">Featured Brands</h1> <a style="display: inline;" href="">View All</a>
    </div>
    <div id="carouselExampleControls" style="margin-top: 0px;" class="carousel slide" data-ride="carousel">
        <div class="carousel-inner">
            <div class="carousel-item active">
                <div class="row">
                    <div class="col-md-4">

                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>



                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/2.jpg" alt="First slide">
                            <div class="card-body">

                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/3.jpg" alt="First slide">
                            <div class="card-body">

                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>


                </div>

            </div>
            <div class="carousel-item">
                <div class="row">
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>


                </div>
            </div>
            <div class="carousel-item">
                <div class="row">
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card" style="width: 27rem;">
                            <img class="d-block w-100" style="height:230px;object-fit:cover" src="public/clientassets/images/1.jpg" alt="First slide">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p class="card-text">Some quick</p>
                                    </div>
                                    <div style="text-align: right;" class="col-md-6">
                                        <a href="#" class="btn btn-primary">Shop Now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>


                </div>
            </div>
        </div>
        <a class="carousel-control-prev" href="#carouselExampleControls" role="button" data-slide="prev">
            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
            <span class="sr-only">Previous</span>
        </a>
        <a class="carousel-control-next" href="#carouselExampleControls" role="button" data-slide="next">
            <span class="carousel-control-next-icon" aria-hidden="true"></span>
            <span class="sr-only">Next</span>
        </a>
    </div>

</div>

<div style="border: 0.1px solid gray;background-color: white; margin-top:10px;margin-bottom:10px;box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">


<div class="row" style="margin: 0;">
<div class="col-md-4">
    <div class="">
    <img class="d-block w-100" style="height:130px;object-fit:cover;height:250px;margin-bottom:10px;margin-top:10px" src="public/clientassets/images/1.jpg" alt="First slide">
    </div>
    <div class="">
    <img class="d-block w-100" style="height:130px;object-fit:cover;height:250px;" src="public/clientassets/images/1.jpg" alt="First slide">
    </div>

</div>
<div class="col-md-4">
<img class="d-block w-100" style="height:520px;object-fit:cover;margin-top:10px;margin-bottom:10px" src="public/clientassets/images/1.jpg" alt="First slide">
</div>
<div class="col-md-4">
    <div>
    <img class="d-block w-100" style="height:130px;object-fit:cover;height:250px;margin-top:10px;margin-bottom:10px" src="public/clientassets/images/1.jpg" alt="First slide">
    </div>
    <div>
    <img class="d-block w-100" style="height:130px;object-fit:cover;height:250px;" src="public/clientassets/images/1.jpg" alt="First slide">
    </div>

</div>
</div>
</div>

@endsection